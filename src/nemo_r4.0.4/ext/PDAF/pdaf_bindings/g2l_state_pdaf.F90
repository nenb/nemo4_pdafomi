...
SUBROUTINE g2l_state_pdaf(step, domain_p, dim_p, state_p, dim_l, state_l)
   ...
   REAL(pwp), INTENT(out) :: state_l(dim_l)
   ...
   IF (SIZE(vcoord_conv) < mpi_subd_vert) THEN
      WRITE (*, '(/1x,a59/)') &
         'ERROR: automatic array v_coord in g2l_state_pdaf too small '
      CALL abort_parallel()
   END IF
   ...
   DO idx = 1, dim_l
      ...
      IF (idx <= SIZE(var2d_p_offset)) THEN
         state_l(idx) = state_p(loc_2dvar + var2d_p_offset(idx))
      ELSE ! Compute 3d state variables
         a = idx - SIZE(var2d_p_offset)
         b = MOD(a - 1, dim_vert_l) + 1
         c = (a - b)/dim_vert_l + 1
         state_l(idx) = state_p(mpi_subd_lon*mpi_subd_lat*(vcoord_conv(b) - 1) &
                                + loc_2dvar + var3d_p_offset(c))
      END IF
   END DO
